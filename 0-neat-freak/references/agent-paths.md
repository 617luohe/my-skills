...
下列平台在本工作空间（`.claude`/`.codex`/`.cursor` 并存）使用过，先按路径速查，再按上方通用探测法核对加载顺序：

| 平台 | 规则/指令路径 | Skills 路径 | 记忆机制 |
|---|---|---|---|
| OpenClaw | workspace 的 `AGENTS.md`/`CLAUDE.md`（workspace > project-agent > personal-agent > managed/local > bundled > extra dirs） | `~/.openclaw/skills/<name>/SKILL.md`、项目 `.openclaw/skills/<name>/`、workspace `skills/` | 无独立记忆索引；跨会话信息放项目根 markdown；frontmatter 支持 `metadata.openclaw` 做加载 gating（非必需） |
| OpenCode | `~/.config/opencode/`（全局）、`.opencode/`（项目） | 项目 `.opencode/skills/`、`.claude/skills/`、`.codex/skills/` 都被扫描；全局 `~/.config/opencode/skills/`、`~/.claude/skills/`、`~/.codex/skills/` | 默认无独立记忆系统，跨会话信息放在项目根 markdown |

注意：OpenCode 同时读 Claude Code 和 Codex 的 skills 目录，所以同一技能装在 `~/.claude/skills/` 下三家都能识别；OpenClaw 走自己的 `~/.openclaw/skills/`，需单独安装或用符号链接。这些是已知的本机形态，执行时仍以当前平台实际加载为准。
