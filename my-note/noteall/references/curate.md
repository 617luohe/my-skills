...
更新已有笔记时，按成熟度检查是否晋升，在 diff 中提示，确认后应用：

| 晋升路径            | 触发条件                                                         | 行为                         |
| ------------------- | ---------------------------------------------------------------- | ---------------------------- |
| seed → sapling      | 内容完整（有定义 + 正文 ≥3 段）+ 双链成对 + 至少更新过一次       | 更新 `confidence: sapling`   |
| sapling → evergreen | 长期维护（≥3 次更新或跨月）+ 链接丰富（入链 ≥2）+ 双链成对无断链 | 更新 `confidence: evergreen` |

- 新建笔记保持 `confidence: seed`；历史批量补齐默认 `seed`，不猜测等级。
- 晋升不自动执行：每次更新时检查并提示，用户确认后写入。
