...
name: file-organizer
layer: my-note
description: [内部] 文件整理。5维诊断→生成计划→批量执行（移动/重命名/合并）。由 noteall 路由器调度。
---
...
4. **Report** → 输出操作统计 + 建议 `/my-note/index-keeper` 更新索引
...
- 目标路径冲突 → 暂停询问
- 不修改正文，只移动/重命名
- 排除：系统目录、模板目录、assets/、隐藏目录

## MUST 规则

1. **计划阶段（②）必须一次性展示全部诊断结果，一次确认。** 不逐项确认。
2. **不删除文件，归档 = 移动到 7-Sources/。**
3. **不修改文件正文内容，只移动/重命名/合并。**
4. **目标路径冲突时必须暂停询问。**
