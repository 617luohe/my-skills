...
My Note 子域：Noteall 把外部资料整理并发布到启动时解析并锁定的个人知识库。库级的组织、命名与分发词汇见 [`../CONTEXT.md`](../CONTEXT.md)。
...
Noteall 按 `references/config.yaml` 的解析顺序（当前目录 → `NOTEALL_VAULT` → 宿主本地 `config.local.yaml`）解析一次并锁定的 Obsidian Vault。解析或 `.obsidian/` 校验失败时停止；后续阶段不得重新解析或切换。
_Avoid_: 固定知识库、阶段间回退
...
**内部 Worker**:
承担独立且可复用职责的 My Note skill（`index-keeper`、`vault-publisher`），由 Noteall 在对应阶段加载；description 限定触发范围，用户普通请求不构成触发条件。
_Avoid_: 用户入口、独立流水线
...
- 内部 Worker 由 Noteall 在对应阶段加载，可服务一个或多个流水线阶段。
