#!/usr/bin/env python3
"""Serve the cleanup report with a guarded one-click delete API (macOS + Windows).
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path in ("/", "/index.html"):
            ...
            html = TPL.replace("__REPORT_DATA__", blob).replace(
                "__DELETE_CONFIG__", cfg
            )
    ...
    def do_POST(self):
        ...
        for p in req.get("paths") or []:
            ...
            if rp not in allow:
                self._send(
                    403, json.dumps({"ok": False, "error": "路径不在白名单：%s" % p})
                )
                return
