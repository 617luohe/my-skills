#!/usr/bin/env python3
"""Read-only storage scanner for cleanup skill (macOS + Windows).
...
def du_children(path, min_kb=51200, limit=40):
    ...
    for name in entries:
        ...
        if kb < min_kb:
            continue
        results.append(
            {"name": name, "path": child, "size_kb": kb, "size_h": human(kb)}
        )
...
def system_info_macos():
    ...
    info["disks"] = [
        {"name": "Macintosh HD", "total": total, "used": used, "free": free}
    ]
    return info
...
def scan_macos():
    ...
    for key, path, floor in MAC_TARGETS:
        groups[key] = (
            dev_caches_macos()
            if key == "dev_caches"
            else du_children(path, min_kb=floor)
        )
...
def scandir_children(path, min_kb=51200, limit=40):
    ...
    for name in entries:
        ...
        try:
            kb = (
                os.path.getsize(child)
                if os.path.isfile(child)
                else dir_size_bytes(child)
            ) // 1024
        except (PermissionError, OSError):
            continue
        if kb < min_kb:
