...
6. **复评回环** — FAIL 时自动回传意见清单给修复方。PASS WITH WARNINGS 默认完成验收，不自动回环；报告必须记录 warning ID、严重级别、接受人/时间、未修复理由，并在进入 `/5-版本管理` 前展示剩余 warning。只有用户明确选择修复某条 warning 时，才回传该条进入复评。复评仅在所有阻断项关闭后以 PASS 结束。
