...
输入必须是**已由用户确认的 `tasks.md`**。文件必须逐项给出 Task ID、目标、允许修改的文件、验收标准、依赖关系及任务分支名。

1. 找不到 `tasks.md`、不能证明其已获用户确认，或字段不完整时：**停止，不得拆解、建分支或派发**；回复“请用户显式运行 `/1-规划`，确认生成的 `tasks.md` 后再调用 `/multi-worker`。”
