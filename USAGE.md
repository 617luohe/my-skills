...
| `voice-input`      | 语音输入会话模式：先做保真清洗，再按原意回答或执行                | 用户显式 `/voice-input`     | [SKILL.md](voice-input/SKILL.md)            |
...
| `index-keeper` | 索引维护 Worker：增量更新 `_INDEX.md`、缺失补全、健康检查；由 noteall 在 Curate/Maintain/Publish 阶段加载 | 由 `noteall` 调度 | [SKILL.md](my-note/index-keeper/SKILL.md) |
| `vault-publisher` | 受控发布 Worker：校验 → 同步 → 只暂存 owned paths → commit/push；由 noteall 在 Publish 阶段加载 | 由 `noteall` 调度 | [SKILL.md](my-note/vault-publisher/SKILL.md) |
