...
遵守项目 `CLAUDE.md`「工作哲学·编码」与 `/vocabulary/tdd` 完整定义，此处不重复。
...
使用 `/vocabulary/tdd` 执行完整红-绿-重构循环（步骤、测试策略、MUST 均以 tdd 为准）。本技能额外要求：验证（pytest/mypy/linter）→ 自检 → 交 `/3-检查` 审查契约。
...
1. **遵守 `/vocabulary/tdd` 的 MUST 规则**（RED 不重构、公共接口测行为等）。
2. **遇 HITL 任务时暂停决策。** 展示当前进度和待决策项，等待用户输入后继续。
3. **开发完成后必须进行自检。** 确保测试、类型检查、linter 都通过，代码质量基本合格。
4. **自检完成后交给 `/3-检查` 正式验收。** 必须携带审查基点和需求来源；保持改动未提交，不要跳过验收环节或自动调用 `/5-版本管理`。
