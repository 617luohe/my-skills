...
收到 `/3-检查` 裁决（FAIL 或 PASS WITH WARNINGS）及可追踪意见清单时：

1. **逐条修复** — 对每条意见（`S-n` / `SP-n`）做一个最小修复，一次一条；修复后运行对应测试
2. **重新验证** — 跑完整测试套件 + 类型检查 + linter
3. **交回复评** — 携带原审查基点、需求来源、意见清单和每条意见的"已修复 / 未修复 / 拒绝"标注，交回 `/3-检查` 复评
4. **循环终止** — 直到裁决为 PASS，或你明确接受剩余警告 / 拒绝某条意见（标注理由）
