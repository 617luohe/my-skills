...
修复 + 回归测试通过、调试标签清干净后，一句话报出下一步再执行：

- 修复完成、回归绿 → 转 `/8-版本管理` 保存修复。
- 阶段 6 的复盘发现根因是**架构问题**（以下任一条满足）→ 转 `/6-优化` 作为后续重构输入：
  - **测试接缝缺失**：当前架构阻止写可靠回归测试（如支付逻辑与第三方SDK强耦合）
  - **耦合过深**：修复需要同时修改 N 个模块（N ≥ 3）
  - **职责不清**：模块职责模糊导致bug反复出现同一区域
  - **性能瓶颈结构性**：瓶颈源于架构设计（如同步调用链过长），非局部优化可解决
- 修复触及的接缝需要补更完整的审查 → 转 `/5-检查`。
