...
8. **模板块标题保持原样** — 0--claude 模板规定的规则块 H2（称呼规则/工作哲学/工作流路由/支撑层/记忆约定）不改写、不删除；内容精简可以，标题留作 0--claude 的结构检测标记。**与 0--claude 互斥声明**：支撑层标题允许加括号说明（如 `## 支撑层（不进主线，按信号触发）`），0--claude 按前缀 `## 支撑层` 匹配容忍此类变体，不会重复注入
