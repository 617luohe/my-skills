...
description: >
  ...
  管整个知识库（减法/校准），非本次会话沉淀（→/6-最后整理），也不处理整机磁盘或系统状态。
...
8. **小内核标题一致** — 保留 0--claude 模板的 H2：称呼规则、工作哲学、路由入口、记忆约定、本项目配置。旧 `## 工作流路由` 与 `## 支撑层` 属于 Fat 路由镜像，迁移为单一 `## 路由入口` 指针
9. **不抢刚沉淀的记忆** — `/6-最后整理` 本轮刚写入的记忆（同一会话产出）不毕业、不删、不缩；记忆毕业至少隔一轮会话
...
| 步骤 | 内容     | 核心动作                                                     |
| ---- | -------- | ------------------------------------------------------------ |
| 0    | 尺寸体检 | 用文件读取或元数据能力统计行数/字节，超标先精简              |
| 1    | 盘点现状 | 用文件枚举与内容读取工具盘点 Markdown，标「评估过/要改/不用改」 |
| 2    | 识别变更 | 用变更影响矩阵思考：新事实波及哪些文档层？                   |
| 3    | 实际修改 | 顺序：docs/ → CLAUDE.md → memory。减优于加，毕业优于内部挪腾 |
| 4    | 自检清单 | 尺寸+反膨胀(6项) + 完整性+反漏改(11项)                       |
...
2. **项目文档层**：从项目根递归枚举 Markdown，先排除 `.git/`、vendor/第三方源码、嵌套仓库、部署副本及 cache/generated，再读取 README、CLAUDE.md、docs 与本次目标相关的知识文件
3. **全局配置**：读 `~/.claude/CLAUDE.md` 等
4. **回顾对话**：本次对话产生了什么新事实？

对每个纳入范围的文件标「评估过/要改/不用改」，并列出排除组与理由。
...
- 新增 API/路由 → CLAUDE.md 保持路由指针，仅按实际影响更新 docs integration-guide + architecture
...
完整映射表见 [sync-matrix.md](references/sync-matrix.md)。
...
- **合并优于追加**：新信息改旧条目，先用宿主内容搜索能力查同关键字
