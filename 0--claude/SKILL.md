...
3. **结构化检测，不做模糊匹配。** 按 H2 标题精确匹配判断块是否存在（见下表），不靠 grep 关键词。唯一例外：支撑层标题允许带括号说明后缀（如 `## 支撑层（不进主线，按信号触发）`），按前缀 `## 支撑层` 匹配视为已存在。
...
| 工作哲学   | `## 工作哲学`       | template.md 同名 H2 段（沟通/编码/架构三层）             |
| 工作流路由 | `## 工作流路由`     | template.md 的 `## 工作流路由` + 紧随的 `## 支撑层` 两段 |
| 记忆约定   | `## 记忆约定`       | template.md 同名 H2 段                                   |

检测按整行 H2 标题匹配。除支撑层外，标题被改写视为"缺失"——用户可手动合并，本 skill 不猜测同义标题。支撑层按前缀 `## 支撑层` 匹配，允许带括号说明后缀（如 `## 支撑层（不进主线，按信号触发）`），避免与 0--neat-freak 的标题保留规则冲突或重复注入。

**与 0--neat-freak 互斥声明**：0--neat-freak 可能将标题从 `## 支撑层` 改写为带括号说明的变体（如 `## 支撑层（不进主线，按信号触发）`），本 skill 按前缀 `## 支撑层` 匹配，容忍此类变体，视为已存在，不重复注入。
...
3. 写入 `./CLAUDE.md`
4. 告知：✅ 新建完成（称呼规则 + 工作哲学 + 工作流路由 + 支撑层 + 记忆约定）
...
4. 在第一个 `# Title` 之后按序注入缺失块（跳过 YAML frontmatter），块间空一行，顺序：称呼规则 → 工作哲学 → 工作流路由(+支撑层) → 记忆约定
...
Claude：luohe，当前目录 "data-pipeline"。
       📄 无 CLAUDE.md → 按模板新建
       ✅ 已生成：称呼规则 + 工作哲学 + 工作流路由 + 支撑层

---

你：/0--claude（只缺 工作哲学 + 工作流路由）
Claude：luohe，当前目录 "data-pipeline"。
       检测：称呼规则 ✓、工作哲学 ✗、工作流路由 ✗
       ✅ 已从模板注入 工作哲学 + 工作流路由(+支撑层)
