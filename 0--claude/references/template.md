...
直接说需求，AI 按规模选路径，无需报技能名。拿不准用 `/use-skills <需求>`（自动匹配）。复杂任务默认先 `/1-规划`，规划完成后等待明确执行授权，再分阶段推进。
...
| **中大功能**（新功能、多模块、方案不定） | `/1-规划` → **确认共享理解与执行授权** →（存疑 `/3-原型`）→ `/4-开发` → `/5-检查` →（有 bug `/7-调试`）→ `/8-版本管理` → `/9-最后整理` |
| **接手陌生项目** | `/2-分析` 直接建立模块地图、数据流和关键入口 → 再进上面的环 |
