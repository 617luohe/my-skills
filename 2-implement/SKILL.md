...
3. **TDD 实现** — 用 `/tdd` 逐片完成：规划 → RED → GREEN → 下一片。
4. **验证** — 用项目原生 test/type/lint/build 命令在最终合流态跑完整套件。
5. **自检 + 交接** — 自检是轻量门禁（项目配置的完整 test/type/lint/build 门禁过 + 快速扫重复与命名），然后带**未提交改动**交 `/3-review`，显式传：审查基点（探测 `git symbolic-ref refs/remotes/origin/HEAD`，不写死 `main`）+ 实际 diff 范围、需求来源路径、diff 审查意图。
