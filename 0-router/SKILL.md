---
name: 0-router
description: 技能路由网关：复杂或范围不明时先加载，先判目标对象与副作用，再按方案状态和规模选路径。触发：路由、选路径、不知道用哪个技能、拿不准用哪个。
---

# 0-router — 技能路由器

先判目标对象和副作用，再判方案状态与规模。本文件只写「把人送到哪里」，下游技能的规则留在各自正文。

## 入口判定

按以下顺序只缩小到一个入口；信息不足时只追问当前最能区分的一个问题。

1. **目标对象**：知识库 → `/noteall`；图片、截图或照片 → `/vision-skill`；技能、`CLAUDE.md` 或 agent 文档 → `/writing-for-agents`；配置基础设施、凭据、CI secret 或 cutover → `/wizard`；代码或项目工作 → 继续。
2. **副作用**：只读调查或回答 → 只读调查/直接回答；Git 提交、推送或分支操作 → `/5-git`；文档、记忆或知识库一致性校准 → `/0-neat-freak`；其余代码/文档变更 → 继续。
3. **工作状态**：Bug、性能回退或难复现故障 → `/4-debug`；架构质量或既有 diff 的评估 → `/3-review`；需要收尾交接 → `/6-sum`；需开发或设计 → 继续。
4. **方案与规模**：方案未定先 `/1-plan`；方案已定后按下表选择实现路径。

## 规模路由

方案已定且目标为代码/项目变更时，按规模切入主流程：
...
规模只决定代码/项目变更的实现深度，入口判定已先处理对象、副作用和工作状态：
